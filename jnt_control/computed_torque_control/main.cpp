...
int main(void){
	...
	//Gain Setup
	MatrixNd Kp=MatrixNd::Identity()*100.0;
	MatrixNd Kd=MatrixNd::Identity()*20.0;
	MatrixNd Ki=MatrixNd::Identity();
	...
	while(is_run){
		...
		JVec q_ddot_ref = q_ddot_des+Kd*e_dot+Kp*e+Ki*e_int;
		JVec tau = M*q_ddot_ref+C*q_dot+G;
		e_int += e*dt;
		// Set torques
